...
function game() {
  ...
  let wordArray = new Array(nLetters).fill("_");
  let maxGuesses = 6;
  let stats = {numberOfGuesses: maxGuesses, previouslyGuessed: new Set()};
  while (stats.numberOfGuesses) {
    ...
    if (!foundLetter) stats.numberOfGuesses--;
    stats.previouslyGuessed.add(letter);
    if (wordString === wordArray.join("")) {
